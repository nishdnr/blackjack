...
def destroy_key():
    ...
    global dealer_hand
    global player_hand
    # destroy and recreate the dealer frame
    dealer_card_frame.destroy()
    dealer_card_frame = tkinter.Frame(card_frame, bg='green')
    dealer_card_frame.grid(row=0, column=1, sticky='ew', rowspan=2)
    # destroy and recreate the player frame
...
reset_button = tkinter.Button(button_frame, text='New Game', command=destroy_key)
...
deck = list(cards)
# deck = list(cards) + list(cards) + list(cards) # game with multiple decks
shuffle()
